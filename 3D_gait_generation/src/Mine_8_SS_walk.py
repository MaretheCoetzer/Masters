...
logger.info("Performing Euler rotation")
...
logger.info("STEP1 - Setting position vector")
...
logger.info("STEP2 - Creating system space velocity expressions")
...
logger.info("STEP3 - Generating expressions for kinetic and potential energy")
...
logger.info("Starting EOM simplification")
...
# Lambdify
logger.info("Lambdafy all the things")
...
logger.info("Completed body inertia")
...
logger.info("Completed integration")
...
logger.info("Setting slack variables")
...
# # CONTACTS
# # Leg 1 --------------------------------------------------------------------------------------------------------------
logger.info("Calculating leg contacts")
...
# FRICTION CONE
logger.info("Getting friction cone")
# Leg 1 -------------------------------------------------------------------------------------------------------------
...
logger.info("Getting foot x/y positions")
def def_F1_x(m,n,c):   
    var_list = [m.g,m.m[('body',1)],m.m[('femur',1)],m.m[('tibia',1)],m.m[('femur',2)],m.m[('tibia',2)],m.m[('femur',3)],m.m[('tibia',3)],m.m[('femur',4)],m.m[('tibia',4)],
                    m.l[('body',1)],m.l[('femur',1)],m.l[('tibia',1)],m.l[('femur',2)],m.l[('tibia',2)],m.l[('femur',3)],m.l[('tibia',3)],m.l[('femur',4)],m.l[('tibia',4)],m.l[('bodyWidth',1)],m.l[('legRadius',1)],
                    m.inbx,m.inby,m.inbz,m.infxy,m.intxy,m.infz,m.intz,
...
logger.info("Calculating actuator torque")
...
logger.info("Performing EOM position calculations")
def EOM_x(m,n,c):
...
logger.info("Setting Knee angle constraints")
#Knee 1
...
logger.info("Enforcing contact order")
#Leg 1
...
#Steady State
#--------------------------------------------------------------
logger.info("Setting Initial and End constraints")
...
logger.info("Doing the IPOPT thing")
...
animate.save('../results/3D_Quad.gif', writer='PillowWriter', fps=10)
...
# m.h[i].value*hm*100000
animate.save('../results/XZ_Quad.gif', writer='PillowWriter', fps=10)
...
# m.h[i].value*hm*100000
animate.save('../results/YZ_Quad.gif', writer='PillowWriter', fps=10)

HTML(animate.to_jshtml())

#SAVING PARAMETERS
logger.info("Saving results into results folder")
import pandas as pd
#Physical properties -----------------------------------------------------------------------------------------------
